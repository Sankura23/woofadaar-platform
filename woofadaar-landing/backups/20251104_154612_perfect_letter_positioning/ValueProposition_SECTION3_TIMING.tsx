...
export default function ValueProposition() {
  ...
  return (
    <section
      ref={containerRef}
      className="h-[800vh] relative"
    >
      {/* Fixed container that stays in place - only visible during section 3 */}
      <motion.div
        className="fixed inset-0 h-screen pointer-events-none"
        style={{
          opacity: useTransform(scrollYProgress, [0, 0.1, 0.99, 1], [0, 1, 1, 0])
        }}
      >
        {/* Header Section */}
        <motion.div
          ...
          className="absolute inset-0 bg-primary-beige flex items-center justify-center"
        >
        ...
        {features.map((feature, index) => {
          const startProgress = 0.42 + (index * 0.10);
          ...
          return (
            <motion.div
              ...
              className={`absolute inset-0 ${feature.bgColor} flex items-center`}
            >
            <div className="w-full h-full">
              <div className="grid lg:grid-cols-2 h-full items-center">
                {/* Feature Text */}
                <motion.div
                  className={`${index === 1 || index === 2 ? "lg:order-2" : ""} px-8 lg:px-16 flex items-center justify-center`}
                  style={{
                    y: useTransform(
                      scrollYProgress,
                ...
                {/* Feature Image */}
                <motion.div
                  className={`${index === 1 || index === 2 ? "lg:order-1" : ""} h-full`}
                  style={{
                    scale: useTransform(
                      scrollYProgress,
                      [startProgress - 0.05, startProgress + 0.05, endProgress - 0.05, endProgress + 0.05],
